...
import {
  Users, ArrowLeft, CheckCircle2, AlertTriangle, XCircle, BarChart3, FileText,
} from "lucide-react";
...
const confidenceIcon = { verified: CheckCircle2, partially_verified: AlertTriangle, unverified: XCircle };
...
export default function ComparePage() {
  ...
  const allSkills = Array.from(new Set(selected.flatMap((c) => c.skills.map((s) => s.name))));
  ...
  const getBestForSkill = (skillName: string) => {
    ...
    selected.forEach((c) => {
      ...
      if (skill && skill.score > best) { best = skill.score; bestId = c.id; }
    });
    return bestId;
  };
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="text-center py-8">
                  ...
                  <Link to="/upload"><Button variant="outline" className="mt-3" size="sm">Upload resumes first</Button></Link>
                </div>
              ) : (
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {candidates.map((c) => {
                    ...
                    return (
                      <button key={c.id} onClick={() => !isDisabled && toggleCandidate(c.id)} disabled={isDisabled}
                        className={`flex items-center gap-3 p-3 rounded-lg border text-left transition-all duration-200
                          ${isSelected ? "border-accent bg-accent/5 shadow-elevated" : isDisabled ? "border-border bg-muted/30 opacity-50 cursor-not-allowed" : "border-border bg-card hover:border-accent/40 hover:shadow-card cursor-pointer"}`}>
            ...
            {selected.length >= 2 ? (
              <motion.div key="comparison" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.3 }}>
                <Card className="shadow-card mb-6">
                  <CardContent className="p-0">
                    <div className={`grid ${selected.length === 2 ? "grid-cols-2" : "grid-cols-3"}`}>
                      {selected.map((c, i) => (
                        <div key={c.id} className={`p-6 text-center ${i > 0 ? "border-l border-border" : ""}`}>
                        ...
                        <div key={skillName}>
                          <div className="py-3">
                            ...
                            <div className={`grid ${selected.length === 2 ? "grid-cols-2" : "grid-cols-3"} gap-4`}>
                              {selected.map((c) => {
                                const skill = getSkill(c, skillName);
                                if (!skill) return (<div key={c.id} className="text-xs text-muted-foreground italic flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full bg-muted-foreground/30" />Not listed</div>);
                                ...
                                return (
                                  <div key={c.id} className={`rounded-lg p-2.5 ${isBest ? "bg-accent/5 ring-1 ring-accent/20" : "bg-secondary/30"}`}>
                                    <div className="flex items-center justify-between mb-1.5">
                                      <Badge variant="secondary" className="text-xs gap-1"><Icon className="h-3 w-3" />{confidenceLabel[skill.confidence]}</Badge>
                                      <span className={`text-sm font-bold ${level === "high" ? "score-high" : level === "medium" ? "score-medium" : "score-low"}`}>{skill.score}%</span>
                                    </div>
                                    <div className="h-1.5 w-full rounded-full bg-secondary">
                                      <div className={`h-full rounded-full transition-all duration-700 ${level === "high" ? "bg-score-high" : level === "medium" ? "bg-score-medium" : "bg-score-low"}`} style={{ width: `${skill.score}%` }} />
                                    </div>
                                  </div>
                                );
                              })}
                ...
                <Card className="shadow-card mt-6">
                  <CardHeader className="pb-3"><CardTitle className="text-lg font-display">Quick Summary</CardTitle></CardHeader>
                  <CardContent>
                    <div className={`grid ${selected.length === 2 ? "grid-cols-2" : "grid-cols-3"} gap-4`}>
                      {selected.map((c) => {
                        ...
                        const avg = c.skills.length > 0 ? Math.round(c.skills.reduce((sum, s) => sum + s.score, 0) / c.skills.length) : 0;
                        return (
                          <div key={c.id} className="space-y-2 text-sm">
                            <p className="font-display font-semibold text-foreground">{c.name}</p>
                            <div className="flex justify-between"><span className="text-muted-foreground">Verified Skills</span><span className="font-medium score-high">{verified}/{c.skills.length}</span></div>
                            <div className="flex justify-between"><span className="text-muted-foreground">At Risk</span><span className="font-medium score-low">{atRisk}</span></div>
                            <div className="flex justify-between"><span className="text-muted-foreground">Avg. Score</span><span className="font-medium text-foreground">{avg}%</span></div>
                            <Link to={`/reports/${c.id}`}><Button variant="outline" size="sm" className="w-full mt-2 text-xs">View Full Report</Button></Link>
                          </div>
                        );
                      })}
                    </div>
              ...
              <motion.div key="placeholder" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16 text-muted-foreground">
