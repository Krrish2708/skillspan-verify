...
import {
  Upload, FileText, X, ArrowRight, Loader2, CheckCircle2, AlertTriangle,
} from "lucide-react";
...
export default function BulkUploadPage() {
  ...
  const addFiles = (newFiles: FileList) => {
    const fileArr = Array.from(newFiles).map((f) => ({ file: f, status: "pending" as const }));
  ...
  const processAll = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        setFiles((prev) => prev.map((item, idx) => idx === i ? { ...item, status: "completed", resumeId } : item));
      } catch (err: any) {
      ...
      <main className="flex-1 py-16">
        <div className="container max-w-4xl mx-auto px-4">
          ...
          <Card className="shadow-card mb-6">
            ...
            <CardContent className="space-y-4">
              <div className="grid sm:grid-cols-2 gap-4">
                <div><Label htmlFor="role-title" className="text-sm">Role Title</Label><Input id="role-title" placeholder="e.g. Senior React Developer" value={roleTitle} onChange={(e) => setRoleTitle(e.target.value)} /></div>
                <div><Label htmlFor="exp-range" className="text-sm">Experience Range</Label><Input id="exp-range" placeholder="e.g. 3-5 years" value={experienceRange} onChange={(e) => setExperienceRange(e.target.value)} /></div>
              </div>
              <div><Label htmlFor="jd" className="text-sm">Full Job Description</Label><Textarea id="jd" placeholder="Paste the full job description here to enable relevancy scoring..." value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} rows={5} /></div>
            </CardContent>
          </Card>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            ...
            <input id="bulk-file-input" type="file" accept=".pdf,.doc,.docx,.txt" className="hidden" multiple onChange={handleFileInput} />
              ...
              <CardContent className="space-y-2">
                {files.map((f, i) => (
                  <div key={i} className="flex items-center gap-3 p-3 rounded-lg bg-secondary/50">
                    ...
                    {f.status === "pending" && <Badge variant="secondary" className="text-xs">Pending</Badge>}
                    {f.status === "uploading" && <Badge variant="secondary" className="text-xs"><Loader2 className="h-3 w-3 animate-spin mr-1" />Uploading</Badge>}
                    {f.status === "analyzing" && <Badge variant="secondary" className="text-xs"><Loader2 className="h-3 w-3 animate-spin mr-1" />Analyzing</Badge>}
                    {f.status === "completed" && <Badge className="text-xs bg-score-high/10 text-foreground border-0"><CheckCircle2 className="h-3 w-3 mr-1 score-high" />Done</Badge>}
                    {f.status === "failed" && <Badge variant="destructive" className="text-xs"><AlertTriangle className="h-3 w-3 mr-1" />Failed</Badge>}
                    {f.status === "pending" && <button onClick={(e) => { e.stopPropagation(); removeFile(i); }} className="text-muted-foreground hover:text-foreground"><X className="h-4 w-4" /></button>}
                  </div>
                ))}
                <Button onClick={processAll} disabled={processing || pendingCount === 0}
                  ...
                  {processing ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Processing Resumes...</> : <>Analyze All Resumes <ArrowRight className="ml-2 h-4 w-4" /></>}
                </Button>
              </CardContent>
            </Card>
          )}
