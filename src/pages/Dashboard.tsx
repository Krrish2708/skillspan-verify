...
export default function Dashboard() {
  ...
  const uniqueRoles = useMemo(() => {
    const roles = completedResumes
      .map(r => r.candidate_role)
      .filter(Boolean)
  ...
  // Get verdict from hr_notes
  const getVerdict = (resume: Resume): string => {
    ...
    if (verdict.includes("Strong Hire")) return "strong_hire";
    if (verdict.includes("Hire")) return "hire";
    if (verdict.includes("Maybe")) return "maybe";
  ...
  const filteredResumes = useMemo(() => {
    return completedResumes.filter(resume => {
      if (search) {
        const q = search.toLowerCase();
        const nameMatch = (resume.candidate_name || "").toLowerCase().includes(q);
        const roleMatch = (resume.candidate_role || "").toLowerCase().includes(q);
        if (!nameMatch && !roleMatch) return false;
      }
      if (roleFilter !== "all" && resume.candidate_role !== roleFilter) return false;
      if (scoreFilter !== "all") {
        const score = getWeightedScore(resume);
      ...
      if (verdictFilter !== "all" && getVerdict(resume) !== verdictFilter) return false;
      return true;
    });
  }, [completedResumes, search, roleFilter, scoreFilter, verdictFilter, relevancyWeight]);
  ...
  const hasActiveFilters = search || roleFilter !== "all" || scoreFilter !== "all" || verdictFilter !== "all";
  ...
  const stats = [
    { label: "Total Candidates", value: completedResumes.length, sub: `${resumes.length} uploaded`, icon: Users, iconColor: "text-blue-500", iconBg: "bg-blue-500/10" },
    { label: "High Confidence", value: highConfidenceCount, sub: completedResumes.length > 0 ? `${((highConfidenceCount / completedResumes.length) * 100).toFixed(0)}% rate` : "0% rate", icon: CheckCircle2, iconColor: "text-emerald-500", iconBg: "bg-emerald-500/10" },
    { label: "At Risk", value: atRiskCount, sub: completedResumes.length > 0 ? `${((atRiskCount / completedResumes.length) * 100).toFixed(0)}% flagged` : "0% flagged", icon: AlertTriangle, iconColor: "text-red-500", iconBg: "bg-red-500/10" },
    { label: "Reports Available", value: completedResumes.length, sub: "fully analyzed", icon: FileText, iconColor: "text-accent", iconBg: "bg-accent/10" },
  ];
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 py-10">
        <div className="container max-w-7xl mx-auto px-6">
          ...
          {/* ── Header ── */}
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-10 gap-4">
            <div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {stats.map((stat, i) => (
              <motion.div key={stat.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07, duration: 0.4 }}>
                <Card className="premium-card border-border/60 hover:shadow-elevated hover:-translate-y-0.5 transition-all duration-300">
                  <CardContent className="p-5">
                    ...
                    <p className="text-3xl font-display font-bold text-foreground tabular-nums mb-0.5">{isLoading ? "—" : stat.value}</p>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 relative">
                      <div className="h-2 bg-secondary rounded-full overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-blue-500 to-accent rounded-full transition-all duration-150" style={{ width: `${relevancyWeight}%` }} />
                      </div>
                      <input type="range" min={0} max={100} value={relevancyWeight} onChange={(e) => setRelevancyWeight(Number(e.target.value))} className="absolute inset-0 w-full opacity-0 cursor-pointer h-2" />
                    </div>
                    <div className="w-28">
                  ...
                  <div>
                    ...
                    {completedResumes.length > 0 && (
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {hasActiveFilters ? `${sortedResumes.length} of ${completedResumes.length} candidates` : `${completedResumes.length} candidates analyzed`}
                      </p>
                    )}
                  </div>
                  {completedResumes.length > 0 && (
                    <Button variant="outline" size="sm" className="gap-2 text-xs border-border/60 rounded-xl h-8" onClick={() => setSortHighest(!sortHighest)}>
                  ...
                  <div className="flex flex-wrap items-center gap-2 pb-4 border-b border-border/40">
                    ...
                    <div className="relative flex-1 min-w-[180px] max-w-xs">
                      ...
                      <Input
                        ...
                        className="pl-8 h-8 text-xs rounded-xl border-border/60"
                      />
                    </div>
                    ...
                    <select
                      ...
                      className="h-8 text-xs rounded-xl border border-border/60 bg-background px-2.5 text-foreground cursor-pointer focus:outline-none focus:ring-1 focus:ring-accent"
                    >
                    ...
                    {/* Clear filters */}
                    {hasActiveFilters && (
                      <Button variant="ghost" size="sm" onClick={clearFilters} className="h-8 text-xs text-muted-foreground hover:text-foreground gap-1.5 rounded-xl px-2.5">
              ...
              <CardContent className="px-6 pb-6 pt-4">
                {isLoading ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-sm font-medium text-foreground mb-1">No candidates match your filters</p>
                    <p className="text-xs text-muted-foreground mb-4">Try adjusting or clearing the filters</p>
                    <Button variant="outline" size="sm" onClick={clearFilters} className="rounded-xl border-border/60 text-xs">
                      Clear all filters
                    </Button>
                  </div>
                ) : (
                      ...
                      {sortedResumes.map((resume, i) => {
                        ...
                        return (
                          <motion.div key={resume.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}>
                            <Link to={`/reports/${resume.id}`} className="grid grid-cols-12 gap-4 items-center px-4 py-3.5 rounded-xl hover:bg-secondary/40 transition-all duration-200 group border border-transparent hover:border-border/40">
                              ...
                              <div className="col-span-2 flex justify-center">
                                <span className={`text-xs font-bold tabular-nums px-2.5 py-1 rounded-lg ${(resume.ats_score || 0) >= 75 ? "bg-emerald-500/10 text-emerald-600" : (resume.ats_score || 0) >= 50 ? "bg-yellow-500/10 text-yellow-600" : "bg-red-500/10 text-red-500"}`}>
                                  {resume.ats_score || 0}
                                </span>
                              </div>
                              <div className="col-span-2 flex justify-center">
                                <span className={`text-xs font-bold tabular-nums px-2.5 py-1 rounded-lg ${(resume.relevancy_score || 0) >= 75 ? "bg-blue-500/10 text-blue-600" : (resume.relevancy_score || 0) >= 50 ? "bg-blue-500/10 text-blue-500" : (resume.relevancy_score || 0) === 0 ? "bg-secondary text-muted-foreground" : "bg-red-500/10 text-red-500"}`}>
                                  {resume.relevancy_score || 0}
                                </span>
                              </div>
                              <div className="col-span-2 flex justify-center">
                                <span className={`text-xs font-bold tabular-nums px-2.5 py-1 rounded-lg ${(resume.credibility_score || 0) >= 75 ? "bg-purple-500/10 text-purple-600" : (resume.credibility_score || 0) >= 50 ? "bg-purple-500/10 text-purple-500" : "bg-red-500/10 text-red-500"}`}>
                                  {resume.credibility_score || 0}
                                </span>
                              </div>
                              <div className="col-span-1 flex justify-center">
                                <span className={`text-xs font-bold tabular-nums px-2.5 py-1 rounded-lg ${level === "high" ? "bg-emerald-500/10 text-emerald-600" : level === "medium" ? "bg-yellow-500/10 text-yellow-600" : "bg-red-500/10 text-red-500"}`}>
                                  {weightedScore}
                                </span>
                              </div>
                            </Link>
